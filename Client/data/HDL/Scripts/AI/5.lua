...
function idle:Enter()
	self.time = 0.1;--math.random();
	monster:PlayBase("Idle");
	
	self.playerPos = vector:New(player:GetPos());
end
...
function idle:IsPlayerMoving()
	local curPos = vector:New(player:GetPos());
	local dis = self.playerPos:Sub(curPos);
	self.playerPos = curPos;
